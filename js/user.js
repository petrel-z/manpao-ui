...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const userComments = [
        {
            ...
            // 商品信息
            productName: "麻辣香锅套餐",
            productImage: "https://images.unsplash.com/photo-1565299624946-b28f40a0ca4b?w=80&h=80&fit=crop",
            // 商家信息
            shopName: "川味小厨",
            shopImage: "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=60&h=60&fit=crop",
            shopAddress: "学府路123号",
            shopCategory: "川菜",
            // 评价信息
            rating: 5,
        ...
        },
        {
            ...
            // 商品信息
            productName: "招牌牛肉面",
            productImage: "https://images.unsplash.com/photo-1569718212165-3a8278d5f624?w=80&h=80&fit=crop",
            // 商家信息
            shopName: "老北京面馆",
            shopImage: "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=60&h=60&fit=crop",
        ...
        },
        {
            ...
            // 商品信息
            productName: "宫保鸡丁盖饭",
            productImage: "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=80&h=80&fit=crop",
            // 商家信息
            shopName: "家常菜馆",
            shopImage: "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=60&h=60&fit=crop",
            shopAddress: "五道口789号",
            shopCategory: "家常菜",
            // 评价信息
            rating: 3,
        ...
        },
        {
            ...
            // 商品信息
            productName: "酸菜鱼套餐",
            productImage: "https://images.unsplash.com/photo-1565299507177-b0ac66763828?w=80&h=80&fit=crop",
            // 商家信息
            shopName: "渝味鱼庄",
            shopImage: "https://images.unsplash.com/photo-1551218808-94e220e084d2?w=60&h=60&fit=crop",
        ...
        },
        {
            ...
            // 商品信息
            productName: "红烧肉套餐",
            productImage: "https://images.unsplash.com/photo-1574484284002-952d92456975?w=80&h=80&fit=crop",
            // 商家信息
            shopName: "妈妈味道",
            shopImage: "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=60&h=60&fit=crop",
    ...
    function updateStats() {
        ...
        const goodComments = userComments.filter(comment => comment.rating >= 4).length;
        const avgRating = userComments.reduce((sum, comment) => sum + comment.rating, 0) / totalComments;
        ...
        commentList.innerHTML = comments.map(comment => `
            <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
                ...
                <div class="flex items-center space-x-3 mb-3 pb-3 border-b border-gray-100">
                    <img src="${comment.userAvatar}" alt="${comment.userNickname}" class="w-12 h-12 rounded-full object-cover border-2 border-gray-100">
                    <div class="flex-1">
                        <div class="flex items-center space-x-2">
                            <h3 class="font-medium text-body-md text-gray-800">${comment.userNickname}</h3>
                    ...
                    <div class="text-right w-[30%]">
                        <div class="text-gray-400 text-caption">${comment.commentTime}</div>
                    </div>
                </div>

                <!-- 评价内容区域 -->
                <div class="mb-3">
                    <p class="text-gray-700 text-body-sm mb-2">${comment.comment}</p>
                    <div class="text-gray-500 text-caption">订单号：${comment.orderNumber}</div>
                </div>

                <!-- 商品信息区域 -->
                <div class="mb-3">
                    <div class="text-gray-800 text-body-sm font-medium">${comment.productName}</div>
                </div>

                <!-- 商家卡片区域 -->
                <div class="flex items-center space-x-3 mb-3 p-3 bg-gray-50 rounded-lg">
                    <img src="${comment.shopImage}" alt="${comment.shopName}" class="w-10 h-10 rounded-lg object-cover">
                    <div class="flex-1">
                        <div class="font-medium text-body-sm text-gray-800">${comment.shopName}</div>
                        <div class="text-gray-500 text-caption">${comment.shopAddress}</div>
                        <div class="text-gray-500 text-caption">${comment.shopCategory}</div>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-2 mt-3">
                    <button class="text-red-500 text-body-sm hover:text-red-600" onclick="deleteComment(${comment.id})">
    ...
    function filterComments() {
        ...
        if (ratingFilter === 'all') {
            ...
            filteredComments = userComments.filter(comment => comment.rating === targetRating);
        }
    ...
    // 绑定事件
    function bindEvents() {
        document.getElementById('rating-filter').addEventListener('change', filterComments);
    }
    ...
    window.deleteComment = function(commentId) {
        if (confirm('确定要删除这条评价吗？')) {
            ...
            if (index > -1) {
                ...
                filteredComments = filteredComments.filter(comment => comment.id !== commentId);
                updateStats();
                renderCommentList(filteredComments);
                showToast('评价已删除');
            }
    ...
    function showToast(message) {
        // 创建toast元素
        const toast = document.createElement('div');
        toast.className = 'fixed top-20 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-lg z-50 text-body-sm';
        toast.textContent = message;
        document.body.appendChild(toast);
...
function initEvaluatePage() {
        ...
        tagButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (selectedTags.includes(tag)) {
                    ...
                    this.classList.remove('border-orange-400', 'bg-orange-50', 'text-orange-600');
                    this.classList.add('border-gray-300', 'text-gray-600');
                } else {
                    // 选择标签
                    selectedTags.push(tag);
                    this.classList.remove('border-gray-300', 'text-gray-600');
                    this.classList.add('border-orange-400', 'bg-orange-50', 'text-orange-600');
                }
    ...
    function initSubmitButton() {
        const submitButton = document.querySelector('button[class*="bg-gradient-to-r"]');
